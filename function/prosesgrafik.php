...
if ($_REQUEST['interval'] == '1') {
	...
	for ($i=1; $i <= $jumlahhari; $i++) { 
		$arr['categories'][]=$i;
		$query="select sum(profit) profit from tbl_penjualan 
			where tipe=0 and day(tanggal) = '$i' and year(tanggal) = '$tahun' and month(tanggal)='$bulan' ";
		$result=mysqli_query($koneksi,$query);
		$data=mysqli_fetch_assoc($result);
		$arr['series'][0]['name']="Offline";
		$arr['series'][0]['data'][]=(int)$data['profit'];
		...
		$query="select sum(profit) profit from tbl_penjualan 
			where tipe=1 and day(tanggal) = '$i' and year(tanggal) = '$tahun' and month(tanggal)='$bulan'";
		$result=mysqli_query($koneksi,$query);
		$data=mysqli_fetch_assoc($result);
		$arr['series'][1]['name']="Online";
		$arr['series'][1]['data'][]=(int)$data['profit'];
